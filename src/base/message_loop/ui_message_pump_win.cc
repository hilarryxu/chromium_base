...
void WinUIMessagePump::InitMessageWnd() {
  ...
  message_hwnd_ =
      ::CreateWindowW(kWndClass, 0, 0, 0, 0, 0, 0, HWND_MESSAGE, 0, hinst, 0);
}

LRESULT CALLBACK WinUIMessagePump::WndProcThunk(HWND hwnd,
                                                UINT message,
                                                WPARAM wparam,
                                                LPARAM lparam) {
  switch (message) {
...
void WinUIMessagePump::DoRunLoop() {
  // IF this was just a simple ::PeekMessageW() loop (servicing all possible
  // work queues), then Windows would try to achieve the following order
  // according to MSDN documentation about ::PeekMessageW with no filter):
  //    * Sent messages
  //    * Posted messages
  //    * Sent messages (again)
  //    * WM_PAINT messages
...
void WinUIMessagePump::WaitForWork() {
  ...
  DWORD result = ::MsgWaitForMultipleObjectsEx(0, NULL, wait_time, QS_ALLINPUT,
                                               MWMO_INPUTAVAILABLE);
...
void WinUIMessagePump::PreProcessMessage(const MSG& msg) {
  FOR_EACH_OBSERVER(UIObserver, observers_, PreProcessMessage(msg));
}
