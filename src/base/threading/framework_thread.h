...
struct FrameworkThreadTlsData {
  ...
  int managed;  // reference counter, the framework thread is managed by
                // ThreadManager
  int managed_thread_id;  // if a thread is managered, this will be the manager
                          // id of it (may not be a thread id)
...
class BASE_EXPORT FrameworkThread : public virtual base::SupportWeakCallback,
 ...
 public:
  ...
  // Do be sure that the current thread is a crt thread (created by
 ...
 protected:
  ...
  // Initilize the tls data, must be called before any tls associated calls on
  // current thread
  static void InitTlsData(FrameworkThread* self);
  // Free the tls data, must be called after any tls associated calls on current
  // thread
 ...
 private:
  // Create a thread with a option, if the option is null, thread will start
